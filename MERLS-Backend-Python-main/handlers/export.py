...
def export_results():
    ...
    response = supabase.table('submissions').select("*").eq('participant_id', participant_id).eq('is_en', is_en).execute()
    ...
    return send_file(excel_file, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True, download_name=f"{participant_id}-report.xlsx")
